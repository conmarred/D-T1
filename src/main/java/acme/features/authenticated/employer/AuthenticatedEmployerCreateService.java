...
public class AuthenticatedEmployerCreateService implements AbstractCreateService<Authenticated, Employer> {
	...
	public boolean authorise(final Request<Employer> request) {
		...
		return !request.getPrincipal().hasRole(Employer.class);
	}
	...
	public void validate(final Request<Employer> request, final Employer entity, final Errors errors) {
		...
		int spamCompany = (int) (Arrays.asList(spamWords).stream().filter(x -> entity.getCompany().toLowerCase().contains(x.toLowerCase().trim())).count() * 100 / spamWords.length);
		int SpamSector = (int) (Arrays.asList(spamWords).stream().filter(x -> entity.getSector().toLowerCase().contains(x.toLowerCase().trim())).count() * 100 / spamWords.length);
		int sumSpam = spamCompany + SpamSector;
		boolean isEmployerSpam = sumSpam <= spam.getThreshold();
		errors.state(request, isEmployerSpam, "sector", "authenticated.employer.error.spam-entity");
	}
